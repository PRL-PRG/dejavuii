...
This is interesting. Almost `45%` of the originals we have are intra-project originals only, i.e. their clones only appear in the same project. These are most likely artefacts from copying, etc. and we can safely remove them from the dataset. 
...
Let's pick the largest one and check what's going on - it is a project with 512 commits that only contains 3 identical commits applied over and over again, so I guess that problem is being solved ([https://github.com/pkudrin/testgitrepo/commits/master](https://github.com/pkudrin/testgitrepo/commits/master))
...
Finally, let's look at how many of the clones are partial, i.e. do not copy everything from the original:

```{r}
x = projectClonesRecords %>% select(cloneId, cloneFiles = files)
x = inner_join(x, projectOriginalsRecords %>% select(cloneId, originalFiles = files), by = "cloneId")
x$ratio = x$cloneFiles / x$originalFiles
LOG("Partial clones (records)", d = nrow(x %>% filter(cloneFiles != originalFiles)), pct = nrow(projectClonesRecords))
```

```{r}
summary(x$ratio)
DT::datatable(x %>% arrange(ratio) %>% head(100))
```

...
DT::datatable(x %>% arrange(desc(count)) %>% top_n(100) %>% mutate(url = A(name, npmPackageUrl(name))) %>% select(url,count), escape = F)
```
...
manualChangesSample = manualChanges %>% sample_n(50)
```
...
```{r}
x = data.frame(
    project = A(manualChangesSample$projecName, manualChangesSample$projectUrl),

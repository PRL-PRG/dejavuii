...
namespace dejavu {
    ...
    namespace {
        ...
        void SaveRepositories(
                std::unordered_set<Repository, RepositoryHash, RepositoryComp> const &repositories) {

            std::string output_path(DataDir.value() + OutputDir.value() +
                                    "/repositories-missing-creation-times.list");

            clock_t timer = clock();
            std::string task =
                    "saving repository information to disk at '" + output_path +
                    "'";
            helpers::StartTask(task, timer);
            ...
            for (Repository const &repository : repositories) {
                file << repository.user << "/" << repository.project
                     << std::endl;

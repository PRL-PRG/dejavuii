 ...
 */
        ...
        void LoadRepositories(
            ...
            std::vector<std::string> paths = helpers::Split(
        ...
        size_t DataCallback(void *data, size_t size, size_t elements,
                            void *user_data) {
        ...
        void
        ParseHttpHeaderProperty(std::string content, CurlMetaData *user_data) {
...
//            std::cerr << "    >> " << s << std::endl;
        ...
        size_t
        HeaderCallback(char *data, size_t size, size_t elements, void *user) {
            CurlMetaData *user_data = (CurlMetaData *) user;
            std::string content = data;

            if (user_data->header_read_fields > 0 &&
                helpers::strip(content) != "") {
        ...
        void DownloadOne(std::string const &url, CurlMetaData &metadata,
                         DownloadData &data) {
            ...
            if (GitHubPersonalAccessToken.value() != "") {
                ...
                curl_easy_setopt(curl, CURLOPT_USERPWD,
                                 GitHubPersonalAccessToken.value().c_str());
            }
        ...
        void Download(
                std::unordered_set<Repository, RepositoryHash, RepositoryComp> const &repository_set) {
            clock_t timer = clock();
            std::string task = STR(
                    "downloading repository info to json files for "
                            << repository_set.size()
                            << " repos");
            size_t good = 0;
            size_t bad = 0;
